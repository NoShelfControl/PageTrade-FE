...
export const getBooks = (search) => {
  return fetch(
    `https://books.googleapis.com/books/v1/volumes?q=${search}&key=${process.env.GOOGLE_API_KEY}`
  )
    .then((res) => res.json())
    .then((json) =>
      json.items.map((book) => ({
...
export const getUserBooks = (id) => {
  return get(`/api/v1/books/user/${id}`).then((books) =>
    books.map((book) => ({
      id: book.id,
      title: book.title,
      author: book.author,
...
export const getAllBooks = () => {
  return get('/api/v1/books/').then((books) =>
    books.map((book) => ({
      id: book.id,
      title: book.title,
      author: book.author,
...
export const getSingleUserBooks = () => {
  return get('/api/v1/books/user/').then((books) =>
    books.map((book) => ({
      id: book.id,
      title: book.title,
      author: book.author,
...
export const getUserActions = (id) => {
  return get(`/api/v1/feed/${id}`).then((books) =>
    books.map((book) => ({
      id: book.id,
...
export const getGlobalActions = () => {
  return get('/api/v1/feed/').then((actions) =>
    actions.map((action) => ({
      id: action.id,
